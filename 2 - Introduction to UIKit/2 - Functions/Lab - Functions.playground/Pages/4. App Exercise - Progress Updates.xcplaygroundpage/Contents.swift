...
progressUpdate(steps: 10, goal: 10_000)
progressUpdate(steps: 4500, goal: 10_000)
progressUpdate(steps: 6000, goal: 10_000)
progressUpdate(steps: 9500, goal: 10_000)
progressUpdate(steps: 11_000, goal: 10_000)


//:  Your fitness tracking app is going to help runners stay on pace to reach their goals. Write a function called pacing that takes four `Double` parameters called `currentDistance`, `totalDistance`, `currentTime`, and `goalTime`. Your function should calculate whether or not the user is on pace to hit or beat `goalTime`. If yes, print "Keep it up!", otherwise print "You've got to push it just a bit harder!"
func isOnPace(currentDistance: Double, totalDistance: Double, currentTime: Double, goalTime: Double) {
    let currentPace = currentDistance / currentTime
    let distanceProjection = currentPace * goalTime
    if distanceProjection >= totalDistance {
        print("Keep it up!")
    } else {
